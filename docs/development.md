...
### `scripts/test.mjs`（69 件、ネットワーク不要）
...
- 行数取得のモード: 自動 / 手動 / 取得しない の挙動と、旧 boolean からの移行。手動は「サイズを取得」「行数を取得」の 2 ボタン
- 行数 / サイズの表示切り替え: 同じ行がサイズで描かれ、切り替えに API を使わないこと
- 手動モードのチェックボックス: 外した行（ディレクトリ配下含む）が取得から外れること、列の上のチェックボックスでの一括切り替え（最新コミット欄が無いページでは帯に出る）
