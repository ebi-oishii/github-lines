...
(function (GHL) {
  ...
  function normalise(stored) {
    ...
    /* The ramp runs from one threshold to the other. Danger below warn is the
       pair entered the wrong way round, and swapping keeps both numbers the
       reader chose; danger equal to warn is no range at all, and the only
       repair is to make one. */
    if (s.dangerLines < s.warnLines) {
      [s.warnLines, s.dangerLines] = [s.dangerLines, s.warnLines];
    }
